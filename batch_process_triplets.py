#!/usr/bin/env python3
"""
Batch process simulation log files with triplets copysets tracking data.
These files are in the triplets_with_copysets_track subdirectory.
"""
...
def generate_output_name(log_file, output_dir):
    ...
    Example: maxReads_100_accessRate_100_dist_Uniform_20251220_220847.txt
             -> maxReads_100_accessRate_100_dist_Uniform_20251220_220847_triplets_status.png
    """
    basename = os.path.basename(log_file)
    # Remove ".txt" suffix, add "_triplets_status.png"
    name = basename.replace(".txt", "_triplets_status.png")
    return os.path.join(output_dir, name)

def main():
    # Use subdirectories under input/
    logs_dir = "input/logs/triplets_with_copysets_track"
    snaps_dir = "input/snaps/triplets_with_copysets_track"
    output_dir = "output"
    script = "track_tube_status_with_triplets.py"
    ...
    print(f"Found {len(pairs)} matching log/snaps file pairs with triplets copysets tracking")
    print("="*70)
