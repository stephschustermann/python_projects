#!/usr/bin/env python3
"""
Script to compare tube status and lost objects between pairwise_with_sha and group_read_pairwise runs.
...
def plot_comparison(sha_data, group_data, params, output_filename):
    """
    Create comparison plots for pairwise_with_sha vs group_read_pairwise approaches.
    ...
    Args:
        ...
        group_data: same structure as sha_data
...
def find_matching_files(base_path):
    """
    Find matching pairs of files between pairwise_with_sha and group_read_pairwise.
    
    Returns:
        list of tuples: [(params, sha_snap, sha_log, group_snap, group_log), ...]
    """
    ...
    for sha_snap in sha_snaps:
        ...
        if sha_logs and group_snaps and group_logs:
            matches.append((params, sha_snap, sha_logs[0], group_snaps[0], group_logs[0]))
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python compare_sha_vs_group.py <base_path> [output_dir]")
        print("\nExample:")
        print("  python compare_sha_vs_group.py . output/comparisons_sha_vs_group")
        print("\nThis will compare all matching files from pairwise_with_sha and group_read_pairwise folders.")
        sys.exit(1)
    
    base_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "output/comparisons_sha_vs_group"
    ...
    # Process each match
    for params, sha_snap, sha_log, group_snap, group_log in matches:
        maxReads, accessRate, dist = params
        print(f"\n{'='*70}")
        ...
        output_filename = os.path.join(output_dir, 
                                      f"comparison_sha_vs_group_maxReads_{maxReads}_accessRate_{accessRate}_dist_{dist}.png")
        ...
        plot_comparison(sha_data, group_data, params, output_filename)
