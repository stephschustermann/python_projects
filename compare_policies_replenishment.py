#!/usr/bin/env python3
"""
Compare 4 different policies with REPLENISHMENT across the same parameters:
- Clustered Random
- Random with SHA
- Copysets (Triplets)
- Tube Replication
...
# Policy folder mappings for REPLENISHMENT
POLICY_FOLDERS = {
    'Clustered Random': 'input/snaps/new_snaps_replanishment_clustered_random',
    'SHA': 'input/snaps/new_snaps_replanishment_sha',
    'Copysets': 'input/snaps/new_snaps_replanishment_triplets_copysets',
    'Tube Replication': 'input/snaps/new_snaps_replanishment_tube_replication'
}
...
def compare_policies(file_groups, output_dir='output/policy_comparisons_replenishment'):
    ...
    for param_pattern, policy_files in sorted(file_groups.items()):
        ...
        fig.suptitle(f'Policy Comparison - WITH REPLENISHMENT\n{param_pattern}', 
        ...
        # Save the figure
        output_filename = f'replenishment_comparison_{param_pattern}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
...
if __name__ == '__main__':
    print("="*80)
    print("POLICY COMPARISON ANALYSIS - WITH REPLENISHMENT")
    print("="*80)
    ...
    if not file_groups:
        ...
        print("\n" + "="*80)
        print(f"Comparison complete! Check output/policy_comparisons_replenishment/ folder")
        print("="*80)
