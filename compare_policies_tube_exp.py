#!/usr/bin/env python3
"""
Compare 4 different policies with TUBE EXPIRATION across the same parameters:
- Clustered Random
- Random with SHA
- Copysets (Triplets)
- Tube Replication
...
# Policy folder mappings for TUBE EXPIRATION
POLICY_FOLDERS = {
    'Clustered Random': 'input/snaps/new_snaps_tube_exp_clustered_random',
    'SHA': 'input/snaps/new_snaps_tube_exp_sha',
    'Copysets': 'input/snaps/new_snaps_tube_exp_triplets_copysets',
    'Tube Replication': 'input/snaps/new_snaps_tube_exp_tube_replication'
}
...
def compare_policies(file_groups, output_dir='output/policy_comparisons_tube_exp'):
    ...
    for param_pattern, policy_files in sorted(file_groups.items()):
        ...
        fig.suptitle(f'Policy Comparison - WITH TUBE EXPIRATION\n{param_pattern}', 
        ...
        # Save the figure
        output_filename = f'tube_exp_comparison_{param_pattern}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
...
if __name__ == '__main__':
    print("="*80)
    print("POLICY COMPARISON ANALYSIS - WITH TUBE EXPIRATION")
    print("="*80)
    ...
    if not file_groups:
        ...
        print("\n" + "="*80)
        print(f"Comparison complete! Check output/policy_comparisons_tube_exp/ folder")
        print("="*80)
