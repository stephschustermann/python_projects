#!/usr/bin/env python3
"""
Batch process simulation log files from triplets_with_sha subdirectory.
These files use the basic tube status tracking (no copysets).
"""
...
def generate_output_name(log_file, output_dir):
    ...
    Example: maxReads_100_accessRate_100_dist_Uniform_20251221_122940.txt
             -> maxReads_100_accessRate_100_dist_Uniform_20251221_122940_sha_status.png
    """
    basename = os.path.basename(log_file)
    # Remove ".txt" suffix, add "_sha_status.png"
    name = basename.replace(".txt", "_sha_status.png")
    return os.path.join(output_dir, name)

def main():
    # Use subdirectories under input/
    logs_dir = "input/logs/triplets_with_sha"
    snaps_dir = "input/snaps/triplets_with_sha"
    output_dir = "output"
    ...
    print(f"Found {len(pairs)} matching log/snaps file pairs from triplets_with_sha")
    print("="*70)
